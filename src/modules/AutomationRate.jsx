...
export default function AutomationRate({ data, updateData }) {
  ...
  const noRehandlingRate = rehandlingTotal > 0 ? (1 - (rehandlingItems / rehandlingTotal)) * 100 : 0
      ...
      <div className="section-card">
        <div className="section-title">
          ...
          <HelpHint title="물류 자동화율">
            ...
            <HintFormula>{`물류 자동화율 = 자동화 적용 Item 수 ÷ 총 Item 수 × 100`}</HintFormula>
            <HintNote>도입과 내수를 구분해 관리할 때는 같은 기준으로 각각 입력해 비교하세요.</HintNote>
          </HelpHint>
        </div>
        <div className="input-grid">
          <div className="input-group">
            <div className="input-label-row"><span className="input-label">총 Item 수</span></div>
          ...
          <div className="result-box full-width" style={{ background: '#A50034' }}>
            <span className="result-box__label">자동화율 = 자동화 적용 Item 수 ÷ 총 Item 수</span>
      ...
      <div className="section-card">
        <div className="section-title">
          ...
          <HelpHint title="Re-Handling율">
            <p>용기 변경 없이 입고 부품을 사용할 수 있는 비율을 산출합니다.</p>
            <HintFormula>{`No Re-Handling율 = (1 − Re-Handling 부품 수 ÷ 입고 부품 수) × 100`}</HintFormula>
            <HintNote>용기가 바뀌면 Re-Handling 작업으로 봅니다.</HintNote>
          </HelpHint>
        </div>
        <div className="input-grid">
          <div className="input-group">
            <div className="input-label-row"><span className="input-label">입고 Item 수</span></div>
            <input className="input-field" type="number" min={0} value={f.rehandlingTotalItems || ''}
              onChange={e => set('rehandlingTotalItems', e.target.value)} placeholder="미입력 시 총 Item 수" />
          </div>
          <div className="input-group">
          ...
          <div className="result-box full-width" style={{ background: '#A50034' }}>
            <span className="result-box__label">No Re-Handling율 = (1 − Re-Handling ÷ 입고 Item) × 100</span>
            <span className="result-box__value">{fmtN(noRehandlingRate, '%', 1)}</span>
          </div>
        </div>
      </div>
    </div>
